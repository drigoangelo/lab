...
def audio_to_text(filename):
    ...
    if fs_orig != desired_sample_rate:
        ...
        fs_new, audio = convert_samplerate(filename, desired_sample_rate)
    else:
...
def main():
    files = os.listdir('/audio/Audios_base')
    for f in files:
        print(f, flush=True)
        subprocess.run(["deepspeech", "--model", "/deepspeech-0.9.3-models.pbmm", "--scorer", "/deepspeech-0.9.3-models.scorer", "--audio", "/audio/Audios_base/" + f])
        print('---', flush=True)
   

#  --scorer deepspeech-0.9.3-models.scorer --audio audio/2830-3980-0043.wav
